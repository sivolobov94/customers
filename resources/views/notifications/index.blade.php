        ...
        <div class="card card-plain">
            ...
            <div class="content table-responsive table-full-width">
                <table class="table table-hover">
                    ...
                    <tr>
                        <td>Ваш товар <a href="/product/{{$notification->data['product_id']}}">
                                {{$notification->data['product_name']}}
                            </a> хочет заказать пользователь
                            <a href="/user/{{$notification->data['id_user_from']}}">
                                {{$notification->data['name_user_from']}}
                            </a>
                            в размере {{$notification->data['quantity']}} {{$notification->data['measure']}}
                            @if($notification->data['comment'])
                                Комментарий:{{$notification->data['comment']}}
                                @endif
